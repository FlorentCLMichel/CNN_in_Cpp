...
void ConvLayer::save(ofstream& file){
	file << size_filters << sep_val << depth_filters << sep_val << num_filters << sep_line;
	save_4d_array(filters, file);
	save_3d_array(last_input, file);
}
		
void ConvLayer::load(ifstream& file){
	file >> size_filters >> sep_val >> depth_filters >> sep_val >> num_filters >> sep_line;
	d4_array_type filters_ = load_4d_array(file);
	auto shape_filters = filters_.shape();
	filters.resize(boost::extents[shape_filters[0]][shape_filters[1]][shape_filters[2]][shape_filters[3]]);
	filters = filters_;
			...
			for(int n = 0; n < nim; n++){
				for(int ii = 0; ii < size_filters; ii++){
					for(int jj = 0; jj < size_filters; jj++){
						output[f*h_out*w_out + i*w_out + j] += input[n*(h_out+size_filters-1)*(w_out+size_filters-1) + (i+ii)*(w_out+size_filters-1) + (j+jj)] * filters[f*nim*size_filters*size_filters + n*size_filters*size_filters + ii*size_filters + jj];
					}
				}
			}
		}
...
d3_array_type ConvLayer::forward(d3_array_type &input)
{
	...
	d3_array_type output(boost::extents[num_filters][h_out][w_out]);
	...
	for(int ii=0; ii<size_filters; ii++){
		for(int jj=0; jj<size_filters; jj++){
			for(int n=0; n<nim; n++){
				for(int i=0; i<h_out; i++){
					for(int j=0; j<w_out; j++){
						d_L_d_filters[(f*nim + n)*size_filters*size_filters + ii*size_filters + jj] += d_L_d_out[f*h_out*w_out + i*w_out + j] * last_input[n*h*w + (i+ii)*w + j+jj];
						d_L_d_input[n*h*w + (i+ii)*w + j+jj] += d_L_d_out[f*h_out*w_out + i*w_out + j] * filters[(f*nim + n)*size_filters*size_filters + ii*size_filters + jj];
					}
				}
			}
		}
...
d3_array_type ConvLayer::backprop(d3_array_type &d_L_d_out, double learn_rate){
	...
	// get the number of images, height, and wifth of the last input
	...
	d4_array_type d_L_d_filters(boost::extents[num_filters][nim][size_filters][size_filters]);
	...
	for(int f=0; f<num_filters; f++){
		for(int z=0; z<nim; z++){
			for(int ii=0; ii<size_filters; ii++){
				for(int jj=0; jj<size_filters; jj++){
					filters[f][z][ii][jj] -= learn_rate * d_L_d_filters[f][z][ii][jj];
				}
			}
		}
	}
