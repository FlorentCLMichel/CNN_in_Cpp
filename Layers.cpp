...
// define types for 2d to 4d arrays
typedef boost::multi_array<double, 2> d2_array_type;
typedef boost::multi_array<double, 3> d3_array_type;
typedef boost::multi_array<double, 4> d4_array_type;

// converts a 4d numpy array to a boost multiarray
d4_array_type d4_numpy_to_multi_array(np::ndarray a){
	int h = a.shape(0);
	int w = a.shape(1);
	int p = a.shape(2);
	int q = a.shape(3);
	d4_array_type A(boost::extents[h][w][p][q]);
	for(d4_array_type::index i = 0; i<h; i++){
		for(d4_array_type::index j = 0; j<w; j++){
			for(d4_array_type::index k = 0; k<p; k++){
				for(d4_array_type::index l = 0; l<q; l++){
					A[i][j][k][l] = p::extract<double>(a[i][j][k][l]);
				}
			}
		}
	}
...
// converts a 4d array to a numpy array
np::ndarray d3array_to_numpy(d4_array_type A){
	const long unsigned int* shape_A = A.shape();
	p::tuple shape = p::make_tuple(shape_A[0], shape_A[1], shape_A[2], shape_A[3]);
...
// write a 4d array to a file (shape followed by data), with entries preceded 
// by '*' except the first value of the shape, and print a '$' at the end of 
// the shape and data
void save_4d_array(d4_array_type &arr, ofstream& file){
	auto shape = arr.shape();
	file << shape[0] << sep_val << shape[1] << sep_val << shape[2] << sep_val << shape[3] << sep_line;
	for(int i=0; i<shape[0]; i++){
		for(int j=0; j<shape[1]; j++){
			for(int k=0; k<shape[2]; k++){
				for(int l=0; l<shape[3]; l++){
					file << sep_val << arr[i][j][k][l];
				}
			}
		}
	}
...
// load a 4d array from a file
d4_array_type load_4d_array(ifstream& file){
	int h;
	int w;
	int d;
	int q;
	char c;
	file >> h >> c >> w >> c >> d >> c >> q >> c;
	d4_array_type res(boost::extents[h][w][d][q]);
	double x;
	for(int i=0; i<h; i++){
		for(int j=0; j<w; j++){
			for(int k=0; k<d; k++){
...
// A ReLU layer taking a vector input
class ReLU_v{
	...
	public: 
		...
		// backpropagation
		// d_L_d_out: gradient of the loss function against the output
		vector<double> backprop(vector<double> &d_L_d_out);
...
// A standard fully-connected layer with ReLU activation function
class FullCon{
